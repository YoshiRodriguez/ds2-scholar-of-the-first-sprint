...
import RunbackCardOld from "./RunbackCardOld";
...
function RunbackList() {
	...
	const filteredBosses = bossRunbacks
		.filter((boss) =>
			boss.title.toLowerCase().includes(searchQuery.trim().toLowerCase()),
		)
		.filter(
			(boss) =>
				difficultyLevel.trim().toLowerCase() === "all" ||
				boss.difficulty
					.toLowerCase()
					.includes(difficultyLevel.trim().toLowerCase()),
		);
		...
		<div>
			...
			>
				...
				<select
					className="ds-input"
					onChange={(m) => {
						setMaxCards(Number.parseInt(m.target.value, 10));
						setStartingCard(0);
					}}
				>
